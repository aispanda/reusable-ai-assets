...
- Node-based consumer verifiers should adapt `stage-cloud-run-release/references/node-cloud-verifier.mjs`: direct `spawnSync('gcloud')` is not portable on Windows. A Firebase Management resource URL must independently name and verify the target project. For a user access token, `x-goog-user-project` attributes quota/billing only; it does not select or isolate the resource. The relevant API must be enabled on that quota project and the caller must already have `serviceusage.services.use`; do not broaden IAM during verification.
...
- If a candidate revision exists but is not Ready, `--candidate` preserves it and tries the next bounded deterministic retry suffix (`-r1` through `-r3`) with the same immutable digest and zero traffic. It never rebuilds, deletes the failed revision, or changes normal staging traffic.
